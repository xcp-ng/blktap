...
int
tap_cli_commit(int argc, char **argv, td_err *error)
{
	const char *args;
	int c, pid, minor, err;
	...
	err = tap_ctl_commit(pid, minor, args);
	if (!err)
		return td_err_set_success(error);
	return td_err_set_errno(error, err);
...
int
tap_cli_query_commit_job(int argc, char **argv, td_err *error)
{
	int c, pid, minor, err;
	...
	err = tap_ctl_query_commit_job(pid, minor);
	if (!err)
		return td_err_set_success(error);
	return td_err_set_errno(error, err);
...
int
tap_cli_cancel_commit_job(int argc, char **argv, td_err *error)
{
	int c, pid, minor, err;
        bool wait;
	...
	err = tap_ctl_cancel_commit_job(pid, minor, wait);
	if (!err)
		return td_err_set_success(error);
	return td_err_set_errno(error, err);
